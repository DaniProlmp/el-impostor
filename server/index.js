...
function createRoom(hostId, hostName) {
  ...
  rooms[code] = {
    ...
    hints: [], votes: {}, eliminated: null, impostorCaught: false, roundTimers: {},
  };
  return rooms[code];
}
...
function startRound(code) {
  ...
  room.wordPair = pair;
  room.impostorId = alive[Math.floor(Math.random() * alive.length)].id;
  room.currentPlayerIdx = 0;
  room.hints = []; room.votes = {};
  room.eliminated = null; room.impostorCaught = false;
  room.phase = "reveal"; room.round += 1;
  emitRoom(code);
}
...
function advanceTurn(code) {
  ...
  if (!room) return;
  const alive = room.players.filter(p => p.connected);
  if (room.hints.length >= alive.length) { room.phase = "vote"; emitRoom(code); return; }
  const hintedIds = new Set(room.hints.map(h => h.playerId));
  const next = room.players.find(p => p.connected && !hintedIds.has(p.id));
  ...
  room.roundTimers[next.id] = setTimeout(() => {
    ...
    if (!r.hints.some(h => h.playerId === next.id)) {
...
function tallyVotes(code) {
  ...
  room.eliminated = eliminated; room.impostorCaught = impostorCaught;
  room.phase = "results"; emitRoom(code);
}
...
socket.on("game:ready", () => {
  ...
  socket.on("game:hint", ({ text }) => {
    ...
    if (room.hints.some(h => h.playerId === socket.id)) return;
    clearTimeout(room.roundTimers[socket.id]);
    room.hints.push({ playerId: socket.id, playerName: currentPlayer.name, text: text?.trim() || "…" });
  ...
  socket.on("game:vote", ({ targetId }) => {
    ...
    const alive = room.players.filter(p => p.connected);
    if (Object.keys(room.votes).length >= alive.length) tallyVotes(currentRoom);
    else emitRoom(currentRoom);
  });
...
socket.on("rejoin", ({ code, name }) => {
  ...
  socket.on("disconnect", () => {
    ...
    if (!room.players.some(p => p.connected)) delete rooms[currentRoom];
    else emitRoom(currentRoom);
  });
});
